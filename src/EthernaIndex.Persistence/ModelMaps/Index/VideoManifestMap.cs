...
namespace Etherna.EthernaIndex.Persistence.ModelMaps.Index
{
    internal sealed class VideoManifestMap : IModelMapsCollector
    {
        public void Register(IDbContextEngine dbContextEngine)
        {
                    ...
                    fixDeserializedModelFunc: (_, m) =>
                    {
                        ...
                        if (!m.ValidationErrors.Any())
                        {
                            ...
                            var sources = m.ExtraElements.TryGetValue("Sources", out var sourcesObj) ?
                                new ExtraElementsSerializer(dbContextEngine).DeserializeValue<List<VideoSourceV1>>(sourcesObj) :
                                new List<VideoSourceV1>();
                            var thumbnail = m.ExtraElements.TryGetValue("Thumbnail", out var thumbnailObj) ?
                                new ExtraElementsSerializer(dbContextEngine).DeserializeValue<ThumbnailV1>(thumbnailObj) :
                                null;
            ...
            dbContextEngine.MapRegistry.AddModelMap<ValidationError>(
                "f555eaa8-d8e1-4f23-a402-8b9ac5930832"); //v0.3.0

            dbContextEngine.MapRegistry.AddModelMap<VideoManifestMetadataBase>(
                "07b4aaa1-c6a2-4dc0-9342-0acd81a2c63e"); //v0.3.9

            //manifest v1
            dbContextEngine.MapRegistry.AddModelMap<VideoManifestMetadataV1>( //v0.3.9
                "8bc43b2f-985b-443c-9a16-e9420a8a1d9d");

            dbContextEngine.MapRegistry.AddModelMap<ThumbnailV1>(
            ...
            dbContextEngine.MapRegistry.AddModelMap<VideoSourceV1>(
                "ca9caff9-df18-4101-a362-f8f449bb2aac"); //v0.3.0

            //manifest v2
            dbContextEngine.MapRegistry.AddModelMap<VideoManifestMetadataV2>(
                "eff75fd8-54ea-437f-862b-782a153416bc"); //v0.3.9

            dbContextEngine.MapRegistry.AddModelMap<ThumbnailV2>(
                "36966654-d85c-455b-b870-7b49e1124e6d"); //v0.3.9

            dbContextEngine.MapRegistry.AddModelMap<ImageSourceV2>(
                "1fbae0a8-9ee0-40f0-a8ad-21a0083fcb66"); //v0.3.9

            dbContextEngine.MapRegistry.AddModelMap<VideoSourceV2>(
        ...
        /// </summary>
        /// <param name="originDelete">Reaction of the referencing documents to the manifest deletion</param>
        public static ReferenceSerializer<VideoManifest, string> PreviewInfoSerializer(IDbContextEngine dbContextEngine, OriginDeleteMode originDelete) =>
        ...
        /// </summary>
        /// <param name="originDelete">Reaction of the referencing documents to the manifest deletion</param>
        public static ReferenceSerializer<VideoManifest, string> ReferenceSerializer(IDbContextEngine dbContextEngine, OriginDeleteMode originDelete) =>
            new(dbContextEngine, config =>
            {
                config.OriginDelete = originDelete;
