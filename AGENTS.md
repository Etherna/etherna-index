...
- **`src/EthernaIndex.Services`** — Application services and side effects. `Domain/` holds services that orchestrate domain operations (`UserService`, `VideoService`). `Infrastructure/` holds `SwarmService`, which fetches and parses published video manifests from the Swarm gateway through the SDK's `IVideoManifestService`: the file contents (preview, details, HLS playlists) are read whole from the gateway's `bzz` endpoint through the SDK's `IUFileProvider` (registered in `ServiceCollectionExtensions` over the same authenticated `ISwarmClient`), while the entry references are resolved chunk by chunk through a `SwarmClientChunkStore`, one mantaray walk per manifest. `EventHandlers/` follows the `On<Event>Then<Action>Handler` convention and is auto-discovered by reflection in `ServiceCollectionExtensions.AddDomainServices` — adding a handler in this namespace registers it automatically. `Tasks/` contains the Hangfire tasks (`VideoManifestValidatorTask`, `RebuildElasticIndexesTask`) and `Queues.cs`. `Options/`/`Settings/` hold configuration objects (`SwarmOptions`, `SsoServerSettings`).
...
- **Implicit lazy loads throw.** Both db contexts run with `ImplicitLazyLoad = ReactionMode.Throw` (`Program.cs`): reading a member a summary doesn't carry throws `ScriniumLazyLoadingException` instead of querying silently. The denial covers the reads: a write completing a summary it has to persist always loads, and fails when the origin document is missing. Summaries carry: `User` references only `SharedInfoId`; `Video` references nothing; `Video.LastValidManifest` carries `IsValid`, `ManifestReference`, `CreationDateTime`; `Video.VideoManifests` items and `UnsuitableVideoReport.VideoManifest` only the id. Before reading anything else, preload explicitly with `dbContext.LoadValuesAsync(models, m => m.Member, ...)` (one `$in` query per repository): the API handler helpers `LoadLastValidManifestsAsync`/`LoadVideoManifestsAsync`, `ElasticSearchService.AddVideoAsync` and `VideoManifestValidatorTask` show the pattern. The `Video` aggregate methods `AddManifest`, `FailedManifestValidation` and `SucceededManifestValidation` read `ManifestReference`, `IsValid` and `CreationDateTime` of every `VideoManifests` item: preload them before calling those methods on a loaded video.
- **The admin panel reports the external services the index calls.** `/admin/services` (`Areas/Admin/Pages/Services/`) shows, for the SSO server and the Etherna Gateway, the configured address, whether it answers and whether the index is accepted as an authenticated client: `Areas/Admin/Services/ExternalServiceChecker` probes the SSO discovery document and asks it for a client credentials token, and probes the gateway health endpoint and reads back the credit of the paying account through the authenticated `CommonConsts.GatewayHttpClientName` client. Every probe is live, with a 10 seconds timeout, so the page tells the state now and never that of a cached answer; the page re-runs them while it is open, every 30 seconds by default. A `404` on the gateway api is reported as such, because it is what an address pointing at a bare bee node answers.
...
- **Log events carrying a Mongo driver exception are filtered before Elasticsearch.** `Program.ConfigureLogging` enriches every event with its destructured exception through `Extensions/LoggerEnrichmentConfigurationExtensions.WithIndexExceptionDetails`, whose `Configs/Logging/MongoExceptionPropertyFilter` keeps only the scalar properties the driver declares on its exceptions (codes, names, messages, flags, error labels), outer or inner, and drops its object graphs (`Command`, `Result`, `Query`, `WriteError`, `ConnectionId`, ...): destructured by reflection they expand into hundreds of nested fields that clash with the stream mapping, and Elasticsearch 9 then answers 201 while redirecting the event to the failure store `logs-ethernaindex-<env>::failures`, invisible in the main stream and to the sink. The sink's own failures (transport errors, hard rejections) reach Serilog's self log, enabled on the console.
