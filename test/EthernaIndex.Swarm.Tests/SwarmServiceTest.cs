    ...
    public class SwarmServiceTest
    {
        ...
        public static IEnumerable<object[]> ParseManifestTests
        {
            get
            {
                ...
                return tests.Select(t => new object[] { JsonSerializer.Deserialize<JsonElement>(t) });
            }
        }

        // Tests.
        [Theory, MemberData(nameof(ParseManifestTests))]
        public void ParseManifest(JsonElement manifestJson)
        {
            // Action.
            var metadata = swarmService.DeserializeMetadataVideo(manifestJson);
